...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("CNN")
    updateDisplayUnit();
}
...
{
    QString ret = QString("Canon:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non CNN unit input in lnReqAmount, we generate the URI with CNN as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::CNN, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
